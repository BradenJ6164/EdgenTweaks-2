...
try {
    ...
    try {
        ...
        //Notifications
         BuildMenuEntry("Notifications", "Send notifications when Edgenuity is wait your response. Refresh your browser after changing.", "Notifications", null, 1)
...
// Notify Pending Action -- baja
function notifyPendingAction() {
    console.log("Notify");

       if (["Vocabulary", "Assignment", "Quiz", "Unit Test Review"].includes(document.getElementById("activity-title").innerText) && ($("#activity-status").text() != "Complete")) {
           (async () => {
   ...
   alert("Edgenuity needs your attention to continue.");
